...
async fn main() {
    ...
    match mode.as_str() {
        "0" => {
            ...
            upload_sender.send(Event::AddPeer {key: public_key1, socket: Framed::new(socket, PineconeCodec)}).await.unwrap();
            let announcement = Frame::TreeAnnouncement(TreeAnnouncement {
                root: Root { public_key: public_key0, sequence_number: 0 },
            ...
            upload_sender.send(Event::SendFrameToPeer {frame: announcement, to: public_key1}).await.unwrap();
        ...
        "1" => {
            ...
            let handle = router.spawn();
            upload_sender.send(Event::AddPeer {key: public_key0, socket: Framed::new(socket, PineconeCodec)}).await.unwrap();
            let announcement = Frame::TreeAnnouncement(TreeAnnouncement {
                root: Root { public_key: public_key1, sequence_number: 0 },
                signatures: vec![RootAnnouncementSignature {
            ...
            upload_sender.send(Event::SendFrameToPeer {frame: announcement, to: public_key0}).await.unwrap();
            let router = handle.await.unwrap();
        }
        _ => {}
    }
